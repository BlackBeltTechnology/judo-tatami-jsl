...
<27> Table prefix used for table names. When `-` is used no prefix is used.
+
(Default: `T_`)

<28> Column prefix used for column names. When `-` is used no prefix is used.
+
(Default: `C_`)

<29> Foreign key prefix used for foreign key names. When `-` is used no prefix is used.
+
(Default: `FK_`)

<30> Inverse Foreign key prefix used for foreign key names. When `-` is used no prefix is used.
+
(Default: `FK_INV_`)

<31> Junction table prefix used for table names. When `-` is used no prefix is used.
+
(Default: `J_`)
